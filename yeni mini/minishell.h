...
/*   Updated: 2025/08/06 18:26:38 by teraslan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_mini
{
	...
	char **tokens;
	int parsing_error;
	int error_printed;  // Flag to prevent multiple error messages
	int last_exit_code;
	int	heredoc_fd;
	t_heredoc	*heredocs;
	t_data *data;
	...
	int in_fd;
	int out_fd;
	pid_t *pids; // For storing process IDs of child processes
	struct s_mini *next;
} t_mini;
